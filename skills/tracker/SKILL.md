...
- Labels: apply `src:cape` to everything cape creates, plus exactly one `type:*` label on tasks and
  ...
  `agent-ticket` to every task and bug sub-issue, never to epics: it marks the issue as an agent
  work ticket whose review surface is the PR, not the issue, so humans can filter these out
  (`-label:agent-ticket`) and review only epics and human-created work. These labels are created by
  the workspace bootstrap — until a given label exists, apply it best-effort and skip what is
