...
a Linear sub-issue through MCP. Load `cape:tracker` and apply its `resources/agent-contract.md` for
create-time rules, including dedupe, labels, priority, naming, and `Done when:`. Then refresh the
epic cache from an MCP `get_issue` result:
