...
4. **Finish.** Stage the resolved files by name with `git add <file>...`. Then, for a merge,
   `cape commit --no-edit` keeps git's merge message. For a rebase, `git rebase --continue` keeps
   the replayed commit's message; repeat from step 1 for each later conflict until every commit is
   rebased.
