...
- End the description with the cache-built closing line, whatever the template source:
  ...
  they are already `Done`, and a listed issue moves to `In Review` when the PR opens. List only ids
  that exist: AI-only work has no `humanTicketId`, so its line starts at the plan issue. Never
  invent a placeholder. Use `Related to` with the same set ONLY when this PR does not complete the
  epic. Build it now, before approval. It is what closes the human ticket and plan issue at merge;
