...
root-cause and reproduction text through it; skip this for pure code or mechanical output.
...
After approval, load `cape:tracker` and apply its Agent contract for create-time rules, including
dedupe, `type:bug`, `src:cape`, `Medium`, a `Fix <symptom>` title, and `Done when:`. Use MCP Linear
`save_issue` to create the bug issue. Include root cause, evidence, reproduction steps, expected
behavior, actual behavior, suggested fix, and success criteria in the description. Then refresh the
cache. If the bug belongs under an epic, refresh the parent epic:
