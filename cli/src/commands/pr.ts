...
const formatValidationErrors = (result: ReturnType<typeof validatePrBody>) => {
  ...
  if (result.missingReviewItem) {
    parts.push(
      'test plan has no code review item: review this branch with /code-review or a cape:code-reviewer pass, then add a checked "- [x] Code review by <reviewer>, findings addressed or dismissed" item to the test plan. The item must name the reviewer; a box that only mentions review does not count',
    );
  }
  return parts.join('; ');
};
...
const prCreate = Command.make(
  ...
  Effect.fn(function* ({ title, body, draft, label, noPush }) {
    if (title.length === 0) {
      return yield* dieWithError('PR title is empty.');
    }
