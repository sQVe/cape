...
const cacheEpic = Command.make(
  ...
  Effect.fn(function* ({ issue }) {
    ...
    yield* Console.log(JSON.stringify({ cached: true, epicId: epic.id, taskCount: epic.tasks.length }));
  }),
).pipe(
...
const cacheTasks = Command.make(
  'cache-tasks',
  {
    epicId: Argument.string('epic-id').pipe(Argument.withDescription('Epic issue id to cache tasks under')),
  ...
  Effect.fn(function* ({ epicId, issues }) {
    ...
    yield* Console.log(JSON.stringify({ cached: true, epicId: trimmedEpicId, taskCount: tasks.length }));
  }),
).pipe(
  Command.withDescription(
...
const cacheStatus = Command.make(
  'cache-status',
  {
    issueId: Argument.string('issue-id').pipe(Argument.withDescription('Issue id whose cached status should update')),
    status: Argument.string('status').pipe(Argument.withDescription('Linear state name, e.g. Todo, In Progress, Done')),
  ...
  Effect.fn(function* ({ issueId, status, stateType }) {
    ...
    yield* Console.log(JSON.stringify({ cached: updatedCache != null, issueId: trimmedIssueId, changed: updatedCache != null }));
  }),
).pipe(Command.withDescription('Refresh one cached issue status after an MCP Linear state update.'));
