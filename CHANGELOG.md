...
- Hooks: the PostToolUse cache-refresh nudge only fired for one Linear MCP server name. The matcher
  ...
  reminder. The matcher is now a regex over the server segment and fires whatever the server is
  called, while still ignoring Linear reads and non-Linear `save_issue` tools.
